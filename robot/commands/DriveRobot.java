...
public class DriveRobot extends CommandBase {
  ...
  public DriveRobot(DoubleSupplier dSpeedIn, DoubleSupplier dTurnIn, DriveTrain objDriveTrainIn, DoubleSupplier dSlowdownIn) {
    objDriveTrain = objDriveTrainIn;
    dsSpeed = dSpeedIn;
    dsTurn = dTurnIn;
    dSlowDown = dSlowdownIn;
  ...
  @Override
  public void execute() {
    if (dSlowDown.getAsDouble() < 0.8) {
      objDriveTrain.westCoastDrive(dsSpeed.getAsDouble(), dsTurn.getAsDouble() / 2.0);
    } else {
      objDriveTrain.westCoastDrive(dsSpeed.getAsDouble() / 1.5 * 0.5, dsTurn.getAsDouble() / 3.0);
    }
    SmartDashboard.putNumber("Drive Speed", dsSpeed.getAsDouble());
    SmartDashboard.putNumber("LTrig", dSlowDown.getAsDouble());
  }
